...
def annmax_jra(reg):
    ' Create annual max dataset from Alans .mat file '
    mat = scipy.io.loadmat('/user/home/hh21501/farfrom/EVT/reg_daily_05_JRA55.mat')
    # Data starts on 01/01/1950. Includes leap days (1952 first leap year)
    # Data starts on 01/01/1958. LY 1960
    daily_reg = mat['reg_daily_05_JRA55']-273.15 # array 237 (regions) x 26163 (days)
    reg_data = daily_reg[reg,:]
    days_in_year = np.tile([365, 365, 366, 365], 16)
    ann_max = []
    for each in np.arange(64): # 72-8 years of data
        year_data = reg_data[sum(days_in_year[:each]):sum(days_in_year[:each+1])]
        ann_max.append(np.max(year_data))
    return ann_max[1:]
...
#    jra_yr = year_max(annmax_jra(reg))
#    era_yr = year_max(annmax_era(reg))
#    if jra_yr != era_yr:
#        count +=1
#    print(reg)
#    print(count)

# Adjusted - 168/237 disagree on year of max
# Before adjusted - 164/237 disagree on year

# ADJUSTING DATA FIRST

list = []
for reg in range(237):
    jra_yr = year_max_multiple_adjust(annmax_jra(reg), 5)
    era_yr = year_max_adjust(annmax_era(reg))
    if era_yr in jra_yr:
        list.append(1)
    else:
        list.append(0)
